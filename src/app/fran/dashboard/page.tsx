      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                  ...
                  <tr key={shop.id} style={{ borderBottom: `1px solid ${s.border}` }}>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => { setEditShop(shop); setEmpCount(shop.employeeCount); setPaidMonths(1); }} style={{ background: s.accent, color: "#fff", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Manage</button>
        ...
        {editShop && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }} onClick={() => setEditShop(null)}>
            <div style={{ background: s.card, border: `1px solid ${s.border}`, borderRadius: 16, padding: 32, width: 400 }} onClick={e => e.stopPropagation()}>
              <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 20 }}>Manage: {editShop.name}</h2>
